  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { priority: 'high', count: tasks.filter(t => t.priority === 'high').length, color: 'red' },
            { priority: 'medium', count: tasks.filter(t => t.priority === 'medium').length, color: 'yellow' },
            { priority: 'low', count: tasks.filter(t => t.priority === 'low').length, color: 'blue' },
          ].map(({ priority, count, color }) => (
            <div key={priority} className="text-center">
              <p className="text-sm text-gray-500 dark:text-gray-400 capitalize">{priority} Öncelik</p>
